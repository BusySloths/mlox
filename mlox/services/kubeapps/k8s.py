...
class KubeAppsService(AbstractService):
    ...
    def teardown(self, conn) -> None:
        ...
        exec_command(
            conn,
            f"helm uninstall {self.release_name} --namespace {self.namespace} --kubeconfig {self.kubeconfig} || true",
            sudo=True,
        )
        # remove namespace
        exec_command(
    ...
    def check(self, conn) -> Dict:
        """
        Checks the status of the KubeApps service, including whether it's terminating.
        """
        helm_error = None
        release_definitely_gone_from_helm = False
