...
def resolve_project(raw: Optional[str]) -> str:
    ...
    if env_value:
        return env_value
    raise typer.BadParameter(f"Provide a project name or set {PROJECT_ENVVAR}.", param_hint="project")

def resolve_password(raw: Optional[str], prompt_text: str = "Password for the session") -> str:
    if raw:
        return raw
...
def resolve_credentials(project: Optional[str], password: Optional[str], prompt_text: str = "Password for the session") -> Tuple[str,str]:
