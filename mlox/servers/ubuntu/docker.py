...
class UbuntuDockerServer(UbuntuNativeServer):
    ...
    def setup_backend(self) -> None:
        ...
        with self.get_server_connection() as conn:  # MyPy will understand this call
            ...
            self.exec.exec_command(conn, "install -m 0755 -d /etc/apt/keyrings", sudo=True)
            self.exec.exec_command(
                conn,
                "curl -fsSL https://download.docker.com/linux/ubuntu/gpg -o /etc/apt/keyrings/docker.asc",
                sudo=True,
            )
            self.exec.exec_command(conn, "chmod a+r /etc/apt/keyrings/docker.asc", sudo=True)
            ...
            self.exec.exec_command(conn, "docker --version", sudo=True)
            self.state = "running"
    ...
    def teardown_backend(self) -> None:
        ...
        with self.get_server_connection() as conn:
            ...
            self.exec.exec_command(conn, "systemctl stop docker", sudo=True, pty=True)
            self.exec.exec_command(conn, "systemctl disable docker", sudo=True, pty=True)
            ...
            self.exec.exec_command(conn, "rm -rf /var/lib/docker", sudo=True)
            self.exec.exec_command(
    ...
    def get_backend_status(self) -> Dict[str, Any]:
        ...
        with self.get_server_connection() as conn:
            ...
            active_result = self.exec.exec_command(
            ...
            enabled_result = self.exec.exec_command(
            ...
            if status_info["docker.is_running"]:
                # Get Docker version
                try:
                    version_json_str = self.exec.exec_command(
                        conn,
                ...
                try:
                    containers_json_str = self.exec.exec_command(
    ...
    def start_backend_runtime(self) -> None:
        with self.get_server_connection() as conn:
            self.exec.exec_command(conn, "systemctl start docker", sudo=True)

    def stop_backend_runtime(self) -> None:
        with self.get_server_connection() as conn:
            self.exec.exec_command(conn, "systemctl stop docker", sudo=True)
