...
| Service | Version(s) | Description | Status |
|---------|-----------|-------------|--------|
| **Developers Terminal Dream** | 0.1-beta | Native-host bootstrap for LazyVim, zsh, oh-my-zsh, Atuin, tmux, Zellij, Midnight Commander, Yazi, LazyVim integrations for Yazi, Zellij, Claude Code, and Codex, Spaceship prompt, git, pyenv-based Python version management, and useful terminal utilities; OpenSSH is expected from the MLOX server bootstrap. Advertised for native, Docker, and Kubernetes-capable backends because all three expose a remote Ubuntu host where developers can use these tools. | Beta |
