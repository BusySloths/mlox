...
- For Multipass server setup, also add the launcher app to **Privacy & Security** -> **Developer Tools**. This includes iTerm2 or Terminal.app for the Textual TUI, VS Code for tests/CLI, Multipass, and Docker or Docker Desktop when Docker-backed services are involved.
...
launches MLOX. Tests and the CLI may work from VS Code while the Textual TUI
fails from iTerm2 if VS Code is allowed but iTerm2 is not. In that case
Multipass can launch the VM, but provisioning steps such as SSH, Docker, or k3s
setup may fail later.
...
3. Enable the apps involved in launching and provisioning:
   ...
   - `Visual Studio Code`, if tests or the CLI run there
